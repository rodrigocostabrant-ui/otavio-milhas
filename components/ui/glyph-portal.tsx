 ...
 *
 * ------------------------------------------------------------------------
 * NOTA DESTE REPO — leia antes de usar.
 *
 * Este componente entra com a câmera por dentro de **tipografia viva**: ele
 * renderiza uma palavra numa fonte, varre o glifo pixel a pixel para achar o
 * maior quadrado de tinta cheia, e amplia por ali até o interior da letra
 * cobrir a tela.
 *
 * A hero do Otávio **não** o instancia, e o motivo é concreto: a marca dele é
 * lettering desenhado, não uma fonte — passar `word="Otavio"` renderizaria
 * Arial Black, que não é a marca de ninguém. O que a hero usa é a mesma
 * técnica de câmera aplicada ao avião da marca, em
 * `components/hero-marca/Voo.tsx` e `beats.ts` (ver `ZOOM`): silhueta como
 * `clip-path`, ponto de fuga no maior quadrado de tinta, ampliação interpolada
 * em espaço logarítmico.
 *
 * Ele fica aqui como origem da técnica e para quem quiser usá-lo em cima de
 * tipografia de verdade em outra página.
 * ------------------------------------------------------------------------
 */
...
export default function GlyphPortal({
  ...
  // Adaptado ao lint deste repo (`react-hooks/immutability` proíbe reatribuir
  // um `let` durante o render). O original acumulava o deslocamento numa
  // variável; aqui ele é derivado. Mesmo resultado, sem mutação.
  const characters = Array.from(text).map((char, i, todos) => ({
    char,
    index: todos.slice(0, i).reduce((n, c) => n + c.length, 0),
  }));
